...
Mode persistence via last_mode.json. Survives reboot.
"""
...
def main():
    ...
    # STEP 2: If was in daemon mode, resume. Otherwise idle-monitor only.
    if saved_mode != 'daemon':
        print('[GLOBAL-LOOP] Mode is dispatch. Idle-monitoring only (no auto-exec).', flush=True)
    while True:
        # Check if we should stay in daemon mode
        ...
        for root in projects:
            ...
            for agent in agents:
                ...
                if rc == 3:  # fuse
                    ...
                    persist_mode('dispatch'); sys.exit(3)
